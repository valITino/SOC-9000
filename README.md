...
Hardware (minimum): 2 vCPU, 2 GB RAM, 20–40 GB disk. Mount pfsense.iso.
Add 5 NICs in this exact order:

    ethernet0 → VMnet8 (WAN)
    ethernet1 → VMnet20 (MGMT)
    ethernet2 → VMnet21 (SOC)
    ethernet3 → VMnet22 (VICTIM)
    ethernet4 → VMnet23 (RED)
...
You’ll see the pfSense console menu (numbered options). Do this:
    ...
    Option 14) Enable Secure Shell → Yes to enable SSH
...
Go back to your PowerShell window (where make up-all paused), press Enter.
The playbooks will SSH in, import config (interfaces, DHCP, rules, syslog), and reboot pfSense automatically.
Expected final IPs:

    MGMT/LAN = 172.22.10.1, SOC = 172.22.20.1, VICTIM = 172.22.30.1, RED = 172.22.40.1.
...
### pfSense (manual, 3 mins)
    ...
    Create VM with 5 NICs: WAN→VMnet8, MGMT→VMnet20, SOC→VMnet21, VICTIM→VMnet22, RED→VMnet23
    Install: Install pfSense → default keymap → Auto (UFS) → Reboot
...
### TheHive ↔ Cortex (once)
    ...
    TheHive → Admin → Cortex → add http://cortex.soc.svc:9001 + API key
